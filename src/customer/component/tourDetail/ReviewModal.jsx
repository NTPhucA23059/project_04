...
export default function ReviewModal({ show, onClose, reviews }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4">
            <div className="bg-white w-full max-w-2xl max-h-[90vh] rounded-xl overflow-hidden shadow-xl flex flex-col">
                        ...
                        {sortedReviews.map((r) => (
                            <div key={r.ReviewID} className="pb-4 border-b last:border-0">
                                <div className="flex items-center gap-3 mb-2">
                                    <div className="w-11 h-11 rounded-full bg-gray-200 flex items-center justify-center text-gray-700 font-bold shadow">
                                        {`U${r.AccountID}`}
                                    </div>

                                    <div>
                                        <p className="font-semibold text-gray-900">User {r.AccountID}</p>
                                        <div className="flex text-yellow-500">
