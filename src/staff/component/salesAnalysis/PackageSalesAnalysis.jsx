...
export default function PackageSalesAnalysis() {
  ...
  const chartData = useMemo(() => {
    ...
    return {
      ...
      topTours: data.topToursByRevenue || [],
    };
  }, [data]);

  const totalRevenue = data ? Number(data.totalRevenue || 0) : 0;
      ...
      {!loading && data && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard
              label="Paid Revenue"
              value={Number(data.paidRevenue || 0)}
            />
                      ...
                      {chartData.topTours.map((tour, idx) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-primary-600">
                            {Number(tour.revenue || 0).toLocaleString()} VND
                          </td>
...
function StatCard({ label, value, accent }) {
  return (
    <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm hover:shadow-md transition">
      ...
      >
        {typeof value === "number"
          ? value.toLocaleString() + (label.includes("Revenue") ? " VND" : "")
          : value}
      </p>
    </div>
  );
        ...
        return (
          <div key={d.label} className="flex-1 flex flex-col items-center">
            <div
              ...
              title={`${d.label}: ${d.value.toLocaleString()} VND`}
            />
